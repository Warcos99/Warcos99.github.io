Below is a transcript of the slides used in a presentation I gave on beginner bookbinding for the Library.


<img src="/Assets/Images/BeginnerBookbinding/cover.jpg" style="height:80vh">

---

# Scope of Program

- Go over tools of the trade
- Upgrades to those tools, as the needs arise
- Touch on different styles of bookbinding schools
	- Book Arts, Bookbinding, Book Restoration
- Discuss the composition of a book and terminology
- Practice simple but high impact techniques
- Create a simple book as an exercise to practice these techniques

---

# Tools of the Trade

The tools needed to produce good quality books are very humble.

- Bone Folder
- Cutting blade
- Metal Ruler
- Needle and Thread
- Awl
- Beeswax
- Bricks (not in this kit)

---

# Tool: the Bone Folder

Most bone folders you get for cheap are made of plastic.

They work great, but they wear down quickly, for a longer lasting tool, try to get a bone folder made from actual bone.

Actual bone folders make it easier to burnish paper.

Burnishing: condensing the fibers of the paper so much, that the surface becomes really smooth and waxy. 

---

# Tool: the Knife

The ideal bookbinding knife has a long straight edge, and is consistently razor sharp.

This makes box cutters a perfect bookbinding knife.
If you wish to upgrade, just get yourself a sturdier and nicer box cutter.

<img src="/Assets/Images/BeginnerBookbinding/knife.jpg" style="height:80vh">

---

# Tool: the Ruler

Ideal ruler
- Made of Metal
- Millimeter Graduations
- Graduations start at the edge

<img src="/Assets/Images/BeginnerBookbinding/ruler.jpg" style="height:80vh">

---

# Tool: Thread

You want **linen** thread that is similar in **thickness to dental floss**. 
If linen is not an option, most threads will suffice so long as it has very little stretch.

As far as thickness:
- the thicker the paper, the thicker the thread.
- Thread that is exposed to the outside should be thicker and heavily waxed

---

# Tool: the Humble Brick

You may need to replace it for a fresh brick in several years, but you won't need to upgrade it

You can use your brick to 
- use as a bookpress
- hold paper in place while you work
- Have a solid surface to fold against
- use two bricks standing up as a paper clamp

---

# Tool: Glue and You

- PH Neutral PVA glue 
	- Good for almost all use cases
	- in some cases, cheaper than Elmer's Glue
- Wheat Starch Paste
	- Make your own paste
	- All you need: Wheat Starch, Water, Clove Oil
	- A batch will last 7 days in the fridge

<img src="/Assets/Images/BeginnerBookbinding/glue.jpg" style="height:80vh">

---

# Other Optional Tools

None of these are necessary to get started, but will make your life easier.  Only consider these once you have a good grasp of the basics.
- Paper Guillotine
- Book Press (Horizontal)
- Book Press (Vertical)
- Ribbon Holder
- Awl Cradle

---

# Bookbinding Styles:

When talking about bookbinding, a lot of people mix these three different schools of crafts.  

- Book Arts
- Traditional bookbinding
- Book Restoration

There are no exact academic definitions for these styles, Here is how I differentiate them.

---

# Book Arts

The umbrella term for all the crafts having to do with creating or working on books

Colloquially you will see it when referring to the most open-ended bookbinding projects with an emphasis on self expression.  While techniques are important, they are not the focus.

<img src="/Assets/Images/BeginnerBookbinding/Bookarts.jpg" style="height:80vh">

---

# Traditional Bookbinding

Classical bookbinding with named techniques tested by time. A large emphasis is placed on craft and tradition, as opposed to self-expression.

<img src="/Assets/Images/BeginnerBookbinding/Bookbinding.jpg" style="height:80vh">

---

# Book Restoration

Can be highly technical, requiring knowledge of historical bookbinding techniques alongside chemical reactions between different materials.

<img src="/Assets/Images/BeginnerBookbinding/BookRestoration.jpg" style="height:80vh">

---

# Anatomy of a Book

- Text Block
- End Papers
- Spine Lining
- Cover Material

<img src="/Assets/Images/BeginnerBookbinding/bookanatomy.jpg" style="height:80vh">

---

# Text Block

- The main body of the book
- The top horizontal edge is called the head
- The bottom edge is called the tail
- Made out of folded papers called signatures

<img src="/Assets/Images/BeginnerBookbinding/textblock.png" style="height:80vh">

---

# End Papers

- Intermediary sheet of paper that attaches text block to cover
- Typically a thicker paper than the text block
- Can be decorative
- Good folding and pasting technique can be implemented to reduce stress on structure

<img src="/Assets/Images/BeginnerBookbinding/endpaper.jpg" style="height:80vh">

---

# Spine Lining Material

- Once a text block is finished, spine is reinforced
- First with glue
- Then with Mull (fabric similar to cheese cloth)
- Depending on book style, tapes or chords might also be used

<img src="/Assets/Images/BeginnerBookbinding/spinelining.jpg" style="height:80vh">

---

# Cover Material

- For hardcover books, a material called chip board is used
- Like all paper, chipboard has grain direction
- Gluing while being conscious about grain reduces warping

<img src="/Assets/Images/BeginnerBookbinding/hardcoverbook.jpg" style="height:80vh">

---

# Tech: Marking Measurements

- Make measurements in millimeters
- Instead of marking a measurement with a pencil, do so with a sharp knife
- The thickness of the knife is much more precise than the thickness of a pencil or pen

<img src="/Assets/Images/BeginnerBookbinding/markingmeasure.jpg" style="height:80vh">

---

# Tech: Making Cuts

- Hold ruler over general area of cut
- Put knife on one of the marks you've made
- Push ruller into knife
- Move knife to another mark
- Push other end of ruler into knife
- Repeat

<img src="/Assets/Images/BeginnerBookbinding/cutting.jpg" style="height:80vh">

---

# Tech: Making Cuts 2

- Apply strong but even pressure down on ruler, so it doesn't move
- on dominant hand, wield the knife. Gently glide the knife across ruler
- Trust the knife is sharp enough, and only apply as much pressure as you need  Applying too much pressure can cause the ruler to move

<img src="/Assets/Images/BeginnerBookbinding/cutting.jpg" style="height:80vh">

---

# Tech: Folding Paper

- Gently bend stack of papers beyond fold point
- then gently bend back until fold point
- This ensures the folds leave a nice even edge

<img src="/Assets/Images/BeginnerBookbinding/edge.jpg" style="height:80vh">

---

# Tech: Punching Holes with Awl

- Hold signature with half of it on table, other half lifted up
- Hold Awl at 45 degrees to the signature
- This ensures all punched holes are even throughout the spine of each signature

<img src="/Assets/Images/BeginnerBookbinding/awl.jpg" style="height:80vh">

---

# Tech: Waxing Thread

- For most cases pre-waxed thread will contain too much wax, making your book really messy
- Get unwaxed thread, and use a bar of beeswax to wax it yourself
- Three or Four runs of the thread through the bar should suffice
- When making a book with thread exposed to the elements, you want alot of more wax

<img src="/Assets/Images/BeginnerBookbinding/wax.jpg" style="height:80vh">

---

# Pamphlet Style Sewing Pattern

<img src="/Assets/Images/BeginnerBookbinding/pamphlet1.png" style="height:80vh">

<img src="/Assets/Images/BeginnerBookbinding/pamphlet2.png" style="height:80vh">

---

# Pamphlet Style Sewing Pattern

<img src="/Assets/Images/BeginnerBookbinding/pamphlet3.png" style="height:80vh">

<img src="/Assets/Images/BeginnerBookbinding/pamphlet4.png" style="height:80vh">

---

# Other Projects: Coptic Stitch

- No gluing required
- Aesthetically Pretty Visible thread on outside
- Book lays flat

<img src="/Assets/Images/BeginnerBookbinding/copticstitch.jpg" style="height:80vh">

---

# Other Projects: French Link Stitch

- For more traditional books
- No ribbons or chords in the spine required
- Sewing provides a lot of structural strength to spine

<img src="/Assets/Images/BeginnerBookbinding/frenchlink.jpg" style="height:80vh">

---

# Other Projects: Perfect Binding

- No sewing required, Binding with only glue
- Used in magazine style binding
- Used to bind single sheets together

<img src="/Assets/Images/BeginnerBookbinding/perfectbinding.jpg" style="height:80vh">

---

# Other Resources

- Non-Adhesive Binding - Keith A. Smith
- Hand Bookbinding - Aldren A. Watson
- Bookbinding: A step by step guide - Kathy Abott
- Bound to Be Beautiful - Zaki Bari
- Bookbinding - Edith Diehl
- DAS Bookbinding (Youtube)
- Four Keys Book Arts (youtube)
